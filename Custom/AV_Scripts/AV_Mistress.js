DMessage("AV_Mistress: start");
main();
DMessage("AV_Mistress: end");
function main()
{
    if (isStrokingAll())
    {
...
{
	if(getVar("AV_Mistress_promise",false) ){
		delVar("AV_Mistress_promise");
		promise();
		return;
	}
	if(getVar("AV_Mistress_youslave",false) ){
		delVar("AV_Mistress_youslave");
		youslave();
		return;
	}
