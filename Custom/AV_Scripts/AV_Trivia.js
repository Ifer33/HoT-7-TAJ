DMessage("AV_Trivia: start");
main();
DMessage("AV_Trivia: end");
function main()
{
...
function how_are_you()
{
    ...
    else if (answer0.isLike("beautiful", "gorgeous", "attractive", "sexy", "adore you", "love you", "amazing", "incredible", "hot"))
    {
        CMessage("Aww... %ThankYou%");
    }
    answer0 = getInput("We don't stop to talk all that often do we?", 8);
    if (answer0.isTimeout())
    {
        talk_more_often();
        return;
    }
    if (answer0.isLike("let's talk","want to talk"))
    {
        CMessage("We definitely should %Smile%");
    }
    else if (answer0.isLike("we should talk", "more often", "love talking"))
    {
    ...
    {
        CMessage("Oh stop it %SubName% *blush*");
        do_you_mean_that();
        return;
    }
    talk_more_often();
...
function talk_more_often()
{
    ...
    if (answer0.isLike("me too", "you too", "too", "well", "likewise"))
    {
...
function anyways_I_should_go()
{
    ...
    if (answer0.isLike("wait","don't go","don't leave","stay")){
