DMessage("pvCr_Punishment: Start");
main();
DMessage("pvCr_Punishment: End");
function main()
{
	if (setVar("pvCr_Punishment_yes_to_spanking"))
	{
		delVar("pvCr_Punishment_yes_to_spanking");
		yes_to_spanking();
		return;
	}
    if (isStroking())
...
function punishment_ending()
{
	...
	increaseAnger(-3);
	setVar("pvInfluenceNeg", getVar("pvInfluenceNeg")+1 ); 
...
{
    CMessage("That's all for today, %PetName%");
	CMessage("I want you to think about what you did");
	CMessage("It's all part of becoming a better submissive for me, %SubName%");
	CMessage("Remember that %Smile%");
	CMessage("I hope to see you again soon!");
    CMessage("Bye");
	setVar("pthevOrgasmLast",2);
    endSession();
    return;
    pvPunishmentThirdEdge();
}
